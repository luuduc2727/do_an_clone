...
const JobDescription = () => {
  ...
  useEffect(() => {
    const fetchSingleJob = async () => {
      try {
        const res = await axios.get(`${JOB_API_URL_ENDPOINT}/getjob/${jobId}`, { withCredentials: true });
        if (res.data.success) {
  ...
  return (
    <div className='max-w-7xl mx-auto my-10'>
      ...
      <h1 className='text-lg font-medium border-b-2 border-b-gray-300 py-4'>Job description</h1>
      <div className='my-4'>
        ...
        <h1 className='font-bold my-1 '> Total Applicants: <span className='pl-4 font-normal text -gray-800 '>{Array.isArray(singleJob?.applications) ? singleJob.applications.length : 0}</span></h1>
